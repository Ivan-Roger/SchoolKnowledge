...
public class ActivityExercise extends AppCompatActivity {
    ...
    public static final String EXTRA_REPONSES = "reponses";
    ...
    private static final String SAVE_REPONSES = "reponses";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState!=null) {
            ...
            reponses = (ArrayList<String>) savedInstanceState.getSerializable(SAVE_REPONSES);
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putSerializable(SAVE_REPONSES,reponses);
    ...
    private void displayResults() {
        int scoreTot = 0;
        ArrayList<ContentValues> resultats = new ArrayList<>();
        int i=0;
        for(Question q: questions) {
            scoreTot+=score.get(i);
            ContentValues p = new ContentValues();
            p.put(ResultViewAdapter.COL_TITLE,q.getTypeName()+"> "+q.getQuestion());
            p.put(ResultViewAdapter.COL_SCORE, score.get(i));
            p.put(ResultViewAdapter.COL_POINTS, q.getScore());
            p.put(ResultViewAdapter.COL_INFO, "Vos réponses: "+reponses.get(i));
            resultats.add(p);
            i++;
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode==REQUEST_QUESTION) {
            if (resultCode==Activity.RESULT_OK) {
                int pts = data.getIntExtra(EXTRA_RESULT,-1);
                score.add(pts);
                reponses.add(data.getStringExtra(EXTRA_REPONSES));
                Log.d(LOG_TAG, "Received activity result => " + pts + "pts.");
                try {
                    startQuestion();
                } catch (Exception e) {
                    Log.e(LOG_TAG,e.getMessage());
